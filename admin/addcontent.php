		...
		for ($currenttab = 0; $currenttab < $numberoftabs; $currenttab++)
		{
			...
			for($i=0;$i<count($contentarray);$i++)
			{
			  if( is_array($contentarray[$i]) ) {
				...
				<div class="pageoverflow" style="<?php if((strpos($contentarray[$i][1],"template_id") !== false) || (strpos($contentarray[$i][1],"cms_dropdown") !== false ) ){echo "display:none";}?>">
					<div class="pagetext">
          <?php if(strpos($contentarray[$i][0],"in_title") !== false ){ 
            echo "<label for='in_title'>*Názov SK:</label>";
          }elseif(strpos($contentarray[$i][0],"in_menutext") !== false ){ 
            echo "<label for='in_menutext'>*Názov EN: </label>";
          }else{ 
            echo $contentarray[$i][0];
          } 
          ?>
          </div>
...
<script type="text/javascript">
  $("select.standard option").eq(1).hide();
  $("select.standard option").eq(2).hide();
  $("select.standard option").eq(3).hide();
  $("select.standard option").eq(4).hide();
  $("select.standard option").eq(5).hide();
  $("select.standard option").eq(8).hide();
</script>
<?php 

if($cur_content_type == "catalogcategory"){ 
?>
<style>
#editab2,#editab3,#edittabpreview{
	display:none;
}
</style>
<?php 
}elseif($cur_content_type == "catalogitem"){
?>
<style>
#editab3,#edittabpreview{
	display:none;
}
</style>
<?php 	
}elseif($cur_content_type == "content"){
?>
<style>
#editab1,#edittabpreview{
	display:none;
}
</style>
<?php 
