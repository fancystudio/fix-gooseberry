...
function content_list_ajax()
{
	...
	$objResponse->script("$('#selectall').hide()");
	$objResponse->script('$(".pagetable tbody tr:eq(0)").hide()');
	$objResponse->script('$(".pagetable tbody #tr_59 .checkbox").hide()');
	$objResponse->script('$(".pagetable tbody #tr_59 .pagepos:eq(4)").css("visibility","hidden")');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2) a").remove()');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2)").append("<span>Referencie</span>")');
	$objResponse->script('$( ".pagetable tbody tr" ).each(function( index ) {
				$(this).find(".pagepos:eq(2)").css("visibility","hidden");
				$(this).find(".pagepos:eq(3)").css("visibility","hidden");
			});    
	');
  $objResponse->script('$("#contentlist form table:eq(0)").find("tr").eq(2).find(".move a").eq(1).hide()');
...
function content_setdefault($contentid)
{
	...
	$objResponse->script("$('#selectall').hide()");
	$objResponse->script('$(".pagetable tbody tr:eq(0)").hide()');
	$objResponse->script('$(".pagetable .checkbox").hide()');
	$objResponse->script('$(".pagetable tbody #tr_59 .pagepos:eq(4)").css("visibility","hidden")');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2) a").remove()');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2)").append("<span>Referencie</span>")');
	$objResponse->script('$( ".pagetable tbody tr" ).each(function( index ) {
        $(this).find(".pagepos:eq(0)").css("visibility","hidden");
				$(this).find(".pagepos:eq(1)").css("visibility","hidden");
				$(this).find(".pagepos:eq(2)").css("visibility","hidden");
				$(this).find(".pagepos:eq(3)").css("visibility","hidden");
			});
	');
...
function content_setactive($contentid)
{
	...
	$objResponse->script("$('#selectall').hide()");
	$objResponse->script('$(".pagetable tbody tr:eq(0)").hide()');
	$objResponse->script('$(".pagetable tbody .checkbox").hide()');
	$objResponse->script('$(".pagetable tbody #tr_59 .pagepos:eq(4)").css("visibility","hidden")');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2) a").remove()');
	$objResponse->script('$(".pagetable tbody #tr_59 td:eq(2)").append("<span>Referencie</span>")');
	$objResponse->script('$( ".pagetable tbody tr" ).each(function( index ) {
  ...
  if ($display != 'none')
      ...
      if( $columnstodisplay['page'] )
	{
	  ...
	  if( $one->MenuText() != CMS_CONTENT_HIDDEN_NAME )
		...
		$txt .= '<a class="tooltip" href="editcontent.php'.$urlext.'&amp;content_id='.$one->Id().'&amp;page='.$page.'" title="'. cms_htmlentities($one->Name().' ('.$one->Alias().')', '', '', true). '" onmouseover="document.getElementById(\'' .$one->Id().'_info\').style.display = \'inline-block\';" onmouseout="document.getElementById(\''.$one->Id().'_info\').style.display = \'none\';">
			' . cms_htmlentities($one->Name()) . '<span id="'.$one->Id().'_info"><strong>'.lang('content_id').':</strong> '.$one->Id().'<br /> <strong>'.lang('title').':</strong> '. cms_htmlentities($one->Name()).'<br /> <strong>'.lang('pagealias').':</strong> '.$one->Alias().'</span></a>';
	      else
      ...
      foreach( $columns as $name => $value )
	{
	  ...
	  switch( $name )
	    ...
	    default:
	      $thelist .= ($name != "alias") ? '<td>'.$value."</td>\n" : "";
	      break;
	    }
	}
      $thelist .= "</tr>\n";
...
function display_content_list($themeObject = null)
{
	...
	if( $columnstodisplay['alias'] )
	  {
	    //$headoflist .= '<th scope="col" title="'.lang('lctitle_alias').'"></th>\n';
	  }
	if( $columnstodisplay['url'] )
	  {
	...
	if( $columnstodisplay['active'] )
	  {
	    $headoflist .= '<th scope="col" title="'.lang('lctitle_active').'" class="pagepos">'.""."</th>\n";
	  }
	if( $columnstodisplay['default'] )
	  {
	    $headoflist .= '<th scope="col" title="'.lang('lctitle_default').'" class="pagepos">'.""."</th>\n";
	  }
	if( $columnstodisplay['move'] )
	  {
...
<?php
	...
	return $headoflist . $thelist . $footer .'</form><script type="text/javascript">
	$(document).ready(function(){	
	$("#selectall").hide();
		$(".pagetable tbody tr:eq(0)").hide();
		$(".pagetable tbody #tr_59 .checkbox").hide();
		});</script></div>';
}
...
?>
		...
		//<![CDATA[
			$("#selectall").hide();
			$(".pagetable tbody tr:eq(0)").hide();
			$(".pagetable tbody .checkbox").hide();
			$(".pagetable tbody #tr_59 .pagepos:eq(4)").css("visibility","hidden");
      $(".pagetable tbody #tr_59 .pagepos:eq(5)").css("visibility","hidden");
			$(".pagetable tbody #tr_59 td:eq(2) a").remove()
			$(".pagetable tbody #tr_59 td:eq(2)").append("<span>Referencie</span>");
			$( ".pagetable tbody tr" ).each(function( index ) {
        $(this).find(".pagepos:eq(0)").css("visibility","hidden");
        $(this).find(".pagepos:eq(1)").css("visibility","hidden");
				$(this).find(".pagepos:eq(2)").css("visibility","hidden");
				$(this).find(".pagepos:eq(3)").css("visibility","hidden");
			});
      $("#contentlist form table:eq(0)").find("tr").eq(2).find(".move a").eq(1).hide();
