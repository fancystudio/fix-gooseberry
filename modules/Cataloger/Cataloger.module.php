...
class Cataloger extends CMSModule
{
  ...
  function storeReferenceFiles($params, $referenceType, $alias){
      
      $db = $this->GetDb();
      if($referenceType == "klient" || $referenceType == "nahlad"){
        $dir = dirname(dirname(dirname(__FILE__)))."/uploads/images/catalog_src";
        if($referenceType == "klient"){
          $referenceTypeDb = "0";
        }else{
          $referenceTypeDb = "2";
        }
      }else{
        $dir = dirname(dirname(dirname(__FILE__)))."/uploads/catalogerfiles/".$alias;
        $referenceTypeDb = "1";
      }
      $dh  = opendir($dir);
      
      $this->deleteReferenceFile($alias, $referenceTypeDb);

      while ($entry = readdir($dh)) {
          $filePartsPodciarkovnik = explode('_',$entry);
          if($filePartsPodciarkovnik[0] == $alias || $filePartsPodciarkovnik[0] == "file"){
              $filePartsBodka = explode('.',$filePartsPodciarkovnik[($referenceType == "klient" || $referenceType == "nahlad") ? 2 : 1]);
               
              $query = 'INSERT INTO '. cms_db_prefix().
              'reference_files (reference_alias, file_name, reference_type, position, desc_sk, desc_en, date_modified) '.
              ' VALUES (?,?,?,?,?,?,?)';
            $dbresult = $db->Execute($query,
              ...
              $entry,  
                    $referenceTypeDb, 
                      ($referenceType == "nahlad") ? null : $params["position".$filePartsBodka[0]], 
                      ($referenceType == "nahlad") ? $params["nahlad".$filePartsBodka[0]] : $params["text_sk".$filePartsBodka[0]], 
                      ($referenceType == "nahlad") ? null : $params["text_en".$filePartsBodka[0]],
                      date("Y-m-d H:i:s")));
          }
  ...
  function selectReferenceFile($fileName, $alias){
      $db = $this->GetDb();
      $dbresult = $db->Execute('SELECT * from '.cms_db_prefix().
			     'reference_files where file_name = ? AND reference_alias = ?',array($fileName, $alias));
      if ($dbresult !== false && $row = $dbresult->FetchRow())
      {   
	       return $row;
      }
  ...
  function selectReferenceCount($alias, $referenceType, $realCount = false){
      if($referenceType == "klient"){
          $referenceTypeDb = "0";
        }elseif($referenceType == "nahlad"){
          $referenceTypeDb = "2";
        }else{
          $referenceTypeDb = "1";
        }
      $db = $this->GetDb();
      $dbresult = $db->Execute('SELECT file_name from '.cms_db_prefix().
			     'reference_files where reference_alias = ? AND reference_type = ?',array($alias, $referenceTypeDb));
	       $imgcount = 0;
         
         while ($dbresult && $row=$dbresult->FetchRow()){
              if(!$realCount){
                $parseFileNameLomitko = explode('_',$row['file_name']);
                
                $parseFileNameBodka = explode('.',$parseFileNameLomitko[($referenceType == "klient" || $referenceType == "nahlad") ? 2 : 1]);
                if($parseFileNameBodka[0] > $imgcount){
                  $imgcount = $parseFileNameBodka[0];  
                }
              }else{
                $imgcount++;
              }
  ...
  function deleteReferenceFile($referenceAlias, $referenceTypeDb){

    $db = $this->GetDb();
    $query = 'DELETE FROM '. cms_db_prefix().
              'reference_files WHERE reference_alias = ? AND reference_type = ?';
    $dbresult = $db->Execute($query, array($referenceAlias, $referenceTypeDb));
  }
...
// more fixes needed _____________________________________________>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  {
  	$config = $this->GetConfig();
  	$srcDir = $config['uploads_path'].$this->getAssetPath('i').'/';
